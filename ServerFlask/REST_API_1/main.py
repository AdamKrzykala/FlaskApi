...
video_put_args.add_argument("name", type=str,
                            help="Name of video", required=True)
video_put_args.add_argument("views", type=int,
                            help="views of video", required=True)
video_put_args.add_argument("likes", type=int,
                            help="Likes of video", required=True)
...
class Video(Resource):
    ...
    def get(self, video_id):
        ...
        if not result:
            abort(404, message="Could not find videos with ID")
        return result
    ...
    def put(self, video_id):
        ...
        result = VideoModel.query.filter_by(id=video_id).first()
        if result:
            abort(409, message="ID occpupied")
    ...
    def patch(self, video_id):
        ...
        result = VideoModel.query.filter_by(id=video_id).first()
        if not result:
            abort(404, message="Does not exists")
